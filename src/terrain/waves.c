...
#define WATER_SIZE 250                  //Map size

static const float STEP_SIZE = 2.5f;       // Wave Size
...
void drawWaves(float time, float playerX, float playerZ)
{
    ...
    for (int x = -size; x < size; x++)
    {
        for (int z = -size; z < size; z++)
        {
            ...
            float h1 = sinf(x * 0.15f + time * 0.7f) * 1.0f +                           //Top left
                       cosf(z * 0.17f + time * 0.5) * 0.65f +
                       sinf((x + z) * 0.06f + time * 0.3f) * 0.35f;

            float h2 = sinf((x + 1) * 0.15f + time * 0.7f) * 1.0f +                     //Top right
                       cosf(z * 0.17f + time * 0.5f) * 0.65f +
                       sinf(((x + 1) + z) * 0.06f + time * 0.3f) * 0.35f;

            float h3 = sinf((x + 1) * 0.15f + time * 0.7f) * 1.0f +                     //Bottom Right
                       cosf((z + 1) * 0.17f + time * 0.5f) * 0.65f +
                       sinf(((x + 1) + (z + 1)) * 0.06f + time * 0.3f) * 0.35f;

            float h4 = sinf(x * 0.15f + time * 0.7f) * 1.0f +                           //Bottom Left
                       cosf((z + 1) * 0.17f + time * 0.5f) * 0.65f +
