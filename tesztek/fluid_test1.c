...
// Macro to map 3D coordinates to a 1D array index
#define IX(x, y, z) ((x) + (y) * N + (z) * N * N)

// --- Data Structures & Setup ---
// These MUST be defined before they are used in functions
...
// --- Global Variables ---
FluidCube *cube;
int N = 32; // Size of the fluid grid

// --- Core Initialization & Memory ---
...
static void advect(int b, float *d, float *d0, float *velocX, float *velocY, float *velocZ, float dt, int N) {
    ...
    float dtx = dt * (N - 2); float dty = dt * (N - 2); float dtz = dt * (N - 2);
    float s0, s1, t0, t1, u0, u1;
    float tmp1, tmp2, tmp3, x, y, z;
    float Nfloat = N; float ifloat, jfloat, kfloat;
    int i, j, k;
    ...
    for(k = 1, kfloat = 1; k < N - 1; k++, kfloat++) {
        for(j = 1, jfloat = 1; j < N - 1; j++, jfloat++) { 
            for(i = 1, ifloat = 1; i < N - 1; i++, ifloat++) {
                ...
                if(x < 0.5f) x = 0.5f; if(x > Nfloat + 0.5f) x = Nfloat + 0.5f; i0 = floorf(x); i1 = i0 + 1.0f;
                if(y < 0.5f) y = 0.5f; if(y > Nfloat + 0.5f) y = Nfloat + 0.5f; j0 = floorf(y); j1 = j0 + 1.0f; 
                if(z < 0.5f) z = 0.5f; if(z > Nfloat + 0.5f) z = Nfloat + 0.5f; k0 = floorf(z); k1 = k0 + 1.0f;
                
                s1 = x - i0; s0 = 1.0f - s1; t1 = y - j0; t0 = 1.0f - t1; u1 = z - k0; u0 = 1.0f - u1;
                
                int i0i = i0; int i1i = i1; int j0i = j0; int j1i = j1; int k0i = k0; int k1i = k1;
...
void FluidCubeStep(FluidCube *cube) {
    ...
    float dt       = cube->dt;
    float *Vx      = cube->Vx; float *Vy      = cube->Vy; float *Vz      = cube->Vz;
    float *Vx0     = cube->Vx0; float *Vy0     = cube->Vy0; float *Vz0     = cube->Vz0;
    float *s       = cube->s; float *density = cube->density;
...
void render() {
    ...
    for(int k = 0; k < N; k++) {
        for(int j = 0; j < N; j++) {
            for(int i = 0; i < N; i++) {
                float d = cube->density[IX(i, j, k)];
                if(d > 0.01f) {
                    float alpha = d > 255.0f ? 1.0f : d / 255.0f;
                    glColor4f(0.1f, 0.5f, 1.0f, alpha); 
                    glVertex3f(i, j, k);
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    // 2. Create an SDL Window with OpenGL context flags
    SDL_Window* window = SDL_CreateWindow(
        "Mike Ash 3D Fluid - SDL2", 
    ...
    // 4. Set up the Fluid Simulation
    cube = FluidCubeCreate(N, 0, 0, 0.1f);
    ...
    while (!quit) {
        ...
        // Add "Geyser" to keep simulation going
        FluidCubeAddDensity(cube, N/2, 2, N/2, 100.0f);
        FluidCubeAddVelocity(cube, N/2, 2, N/2, 0.0f, 3.0f, 0.0f);
