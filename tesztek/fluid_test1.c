...
int main(int argc, char **argv)
{
    ...
    while (!quit)
    {
        ...
        // Inside your main loop (while(!quit))
        // Instead of one pixel, let's add a "blob" of dye
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                // Add a 3x3 area of dye near the bottom center
                FluidCubeAddDensity(cube, N / 4 + i, 4, N / 4 + j, 800.0f);
            }
        }
        // Add a strong upward push
        FluidCubeAddVelocity(cube, N / 2, 2, N / 2, 0.0f, 8.0f, 0.0f);
